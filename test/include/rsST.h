...
#define BC_SC(v) BOOST_CHECK(cd.code == v)
...
class RsST {
public:
	RsST() : s(RsStocks(this->i, this->o)), cP(nullptr), cD(nullptr), tD(nullptr), stocks(s.stocks), codeMap(s.codeMap) {};
	...
	void push_trade_details(string ii) {i << ii << endl; read_trade_details(); stocks.at(td.code).trades.push_back(td);};
        void read_stock_code() {if (cD != nullptr) delete cD; cD = new Code(s.read_stock_code());};
